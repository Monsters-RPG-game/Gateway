    ...
    envKeys.forEach((k) => {
      ...
      switch (key) {
        ...
        case EConfigKeys.CORS_ORIGIN:
          ...
          break;
        case EConfigKeys.HTTP_PORT:
          config[key] = Number(target);
          break;
        case EConfigKeys.SOCKET_PORT:
          config[key] = Number(target);
          break;
        case EConfigKeys.SESSION_SECRET:
          config.session!.secret = target;
          break;
        case EConfigKeys.SESSION_SECURED:
          config.session!.secured = Boolean(target);
          break;
        case EConfigKeys.SESSION_TRUST_PROXY:
          config.session!.trustProxy = Boolean(target);
          break;
        case EConfigKeys.METRICS_LOKI:
          config.metrics!.loki = target;
          break;
        case EConfigKeys.TOKENS_DOMAIN:
